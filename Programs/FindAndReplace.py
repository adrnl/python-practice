...
# The rule is that if x starts at position i in the original string S, then we will replace
...
# overlap in replacement: for example, S = "abc", indexes = [0, 1], sources = ["ab","bc"] is not a valid test case.

# def findReplaceString(input: str, indexes: List[int], sources: List[str], targets: List[str]) -> str:
#     length = len(indexes)
#     if length != len(sources) != len(targets):
#         return input
#
#     for i in reversed(range(length)):
#         if not input.startswith(sources[i], indexes[i]):
#             continue
#         else:
#
#
...
def findReplaceString(input: str, indexes: List[int], sources: List[str], targets: List[str]) -> str:
    ...
    chars = list(input)
    changes = zip(indexes, sources, targets)
    changes.sort()
    for i in reversed(range(length)):
        if not input.startswith(sources[i], indexes[i]):
            continue
            ...
            prelen = len(sources[i])
            chars[indexes[i]:indexes[i]+prelen] = target
